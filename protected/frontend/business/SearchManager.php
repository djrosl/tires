...
class SearchManager extends \common\business\Manager
{
    ...
    {
        $query   = $this->resolveDataProviderQuery($listViewDTO->getSearchModel(), $listViewDTO->getDataCategoryId(), $listViewDTO->getFilterAttributes());
        $sort   = new Sort([
     ...
     * @return \yii\db\Query
     */
    protected function resolveDataProviderQuery($searchModel, $dataCategoryId, $attrs = [])
    {
        ...
        $mappingTable           = UsniAdaptor::tablePrefix() . 'product_category_mapping';
	      $attrTable              = UsniAdaptor::tablePrefix() . 'product_attribute_mapping';
        ...
        if($attrs){
	        ...
	        //$query->join('LEFT JOIN', $attrTable . ' tpam', "tpam.product_id = tp.id");
        	foreach ($attrs as $key => $attr){

        		$query->andWhere("tp.id IN (SELECT product_id FROM $attrTable WHERE attribute_id = $key AND attribute_value = '$attr')");
	        }
        }
