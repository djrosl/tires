...
class SiteController extends BaseController
{
	...
	public function addToCartScript(){
		$url = UsniAdaptor::createUrl('cart/default/add-to-cart');
		$js = "     
                    var dataString = '';
                    $('body').on('click', '.add-cart-rs', function(e){
                    e.preventDefault();
                    ...
                    $.ajax({
                            ...
                            success: function(json) {
                                if (json['success']) {
                                    ...
                                    $('.headerTop-cartVal').attr('data-quant', $(json['data']).find('.headerTop-cartVal').data('quant'));
                                     
                                     swal({
																			  title: 'Товар добавлен!',
																			  type: 'success',
																			  text: 'В корзине '+$(json['data']).find('.headerTop-cartVal').data('quant')+' товаров',
    ...
    public function actionDetail($id) {
    	  ...
    	  $recentProducts = UsniAdaptor::app()->request->cookies->getValue('recent', '');

	      $recentProductsArray = explode(',', $recentProducts);
	      $recent = Product::find()->where(['in', 'id', array_unique($recentProductsArray)])->all();


	      $recentProductsArray[] = $id;

	      UsniAdaptor::app()->response->cookies->add(new Cookie([
					'name'=>'recent',
					'value'=>implode(',', array_unique($recentProductsArray))
				]));
        ...
        $this->view->registerJs($this->addToCartScript(), View::POS_READY);

        //Fetch the result set
        return $this->render("/front/view", ['productDTO' => $productDTO, 'recent'=>$recent]);
    }
